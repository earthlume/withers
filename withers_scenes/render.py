...
Pipeline internals are numpy-vectorized. Scene API is unchanged:
scenes still build frame as a list of 64 (r,g,b) tuples and call pipe.display(frame).
"""
import math
import numpy as np
...
# ---- Color math (scene-facing, unchanged API) --------------------
def lerp(a, b, t):
    t = max(0.0, min(1.0, t))
...
# ---- Drawing primitives (scene-facing, unchanged API) ------------
...
# ---- Render pipeline (numpy-vectorized internals) ---------------
# Bayer 2x2 dither matrix tiled to 8x8.
_BAYER_2x2 = np.array([[0, 2],
                       [3, 1]], dtype=np.int32)
_BAYER_8x8 = np.tile(_BAYER_2x2, (4, 4))  # shape (8,8), values 0..3

# Bloom convolution kernel (3x3, center-0 — bright center bleeds outward only).
_BLOOM_KERNEL = np.array([[0.13, 0.21, 0.13],
                          [0.21, 0.00, 0.21],
                          [0.13, 0.21, 0.13]], dtype=np.float32)

# Luma weights matching legacy `luma` function: (2*R + 3*G + B) / 6.
...
class Pipeline:
    ...
    Scenes still pass `frame` as a list of 64 (r,g,b) tuples. All pipeline work
    is vectorized internally on uint8/float32 (8,8,3) arrays.
    """
    def __init__(self):
        self.sense = SenseHat()
    ...
    @staticmethod
    def _to_array(frame):
        """List of 64 (r,g,b) tuples -> float32 ndarray shape (8,8,3)."""
        arr = np.asarray(frame, dtype=np.float32).reshape(8, 8, 3)
        return arr

    @staticmethod
    def _bloom(arr, threshold=110.0, strength=0.38):
        """Gaussian bleed: bright pixels glow into 3x3 neighborhood.

        Replaces the per-pixel loop with one convolution + masked accumulate.
        """
        # Luma image (8,8)
        L = arr @ _LUMA_W
        # How much each pixel contributes as a bloom source
        excess = np.maximum(L - threshold, 0.0) / 145.0
        np.minimum(excess, 1.0, out=excess)
        excess *= strength  # (8,8)
        if not excess.any():
            return arr
        # Per-channel bloom source = pixel color * its bloom strength
        src = arr * excess[..., None]  # (8,8,3)
        # Convolve each channel with the bloom kernel.
        # Manual convolution for 3x3 kernel — faster to unroll than call scipy.
        pad = np.zeros((10, 10, 3), dtype=np.float32)
        pad[1:9, 1:9] = src
        out = arr.copy()
        k = _BLOOM_KERNEL
        # 8 neighbor offsets (center is 0 in the kernel)
        for dy in (-1, 0, 1):
            for dx in (-1, 0, 1):
                w = k[dy + 1, dx + 1]
                if w == 0.0:
    ...
    def _blur(self, arr, blend=0.70):
        """Temporal blend: blend*current + (1-blend)*previous."""
        out = self._prev + (arr - self._prev) * blend
        self._prev = out
        return out

    def _dither(self, arr):
        """Ordered dither in the deep darks (max channel < 10)."""
        self._dither_phase = (self._dither_phase + 1) & 3
        max_c = arr.max(axis=-1)  # (8,8)
        # Pixel gets zeroed iff phase > bayer threshold AND it's already very dark
        kill = (self._dither_phase > _BAYER_8x8) & (max_c < 10.0)  # (8,8) bool
        arr = arr.copy()
        arr[kill] = 0.0
        return arr
